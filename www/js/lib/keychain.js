define(['js/lib/gibberish-aes', 'js/lib/event_emitter'], function(GibberishAES, EventEmitter) {
  ...
  inherits(Keychain, EventEmitter);
  ...
  Keychain.prototype.setContents = function(itemArray) {
    ...
    for (var itemIndex in itemArray) {
      ...
      if (item.type.indexOf("system.") === 0) continue;
      ...
      if (item.trashed == "Y") {
        ...
        else if (item.type.indexOf(TYPE_FOLDERS) === 0) category = 'folders';
        else if (item.type.indexOf("wallet.membership") === 0) category = TYPE_WALLET;
        else if (item.type.indexOf("wallet.financial") === 0) category = TYPE_WALLET;
        else if (item.type.indexOf("wallet.government") === 0) category = TYPE_WALLET;
      }
...
//    this.updatedAt = date.format("mmm d, yyyy, h:MM:ss TT");
  ...
  function KeychainItem(data) {
    ...
    folderName = this.itemWithUuid(this.folderUuid);
  ...
  KeychainItem.prototype.asHTML = function() {
    ...
    if (this.type.indexOf(TYPE_WALLET) === 0) {
      return this.walletHTML();
    }
